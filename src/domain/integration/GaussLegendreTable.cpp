...
std::vector<QuadraturePoint> GaussLegendreTable::getPoints(int order) {
  std::vector<QuadraturePoint> points;

  switch (order) {
  case 1:
    points.emplace_back(0.0, 2.0, Point2D(0.0, 0.0));
    break;
  case 2: {
    double xi = std::numbers::inv_sqrt3;
    points.emplace_back(-xi, 1.0, Point2D(-xi, 0.0));
    points.emplace_back(xi, 1.0, Point2D(xi, 0.0));
    break;
  }
  case 3: {
    double xi = std::sqrt(3.0 / 5.0);
    points.emplace_back(-xi, 5.0 / 9.0, Point2D(-xi, 0.0));
    points.emplace_back(0.0, 8.0 / 9.0, Point2D(0.0, 0.0));
    points.emplace_back(xi, 5.0 / 9.0, Point2D(xi, 0.0));
    break;
  }
  case 4: {
    ...
    points.emplace_back(-xi2, w2, Point2D(-xi2, 0.0));
    points.emplace_back(-xi1, w1, Point2D(-xi1, 0.0));
    points.emplace_back(xi1, w1, Point2D(xi1, 0.0));
    points.emplace_back(xi2, w2, Point2D(xi2, 0.0));
    break;
  }
  case 5: {
    ...
    points.emplace_back(-xi3, w3, Point2D(-xi3, 0.0));
    points.emplace_back(-xi2, w2, Point2D(-xi2, 0.0));
    points.emplace_back(xi1, w1, Point2D(xi1, 0.0));
    points.emplace_back(xi2, w2, Point2D(xi2, 0.0));
    points.emplace_back(xi3, w3, Point2D(xi3, 0.0));
  ...
  case 8: {
    ...
    for (int i = 0; i < 4; ++i) {
      points.emplace_back(-xi[i], w[i], Point2D(-xi[i], 0.0));
      points.emplace_back(xi[i], w[i], Point2D(xi[i], 0.0));
    }
    break;
  }
